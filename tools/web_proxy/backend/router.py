...
def open_proxy(request: Request, url: str | None = None) -> HTMLResponse | RedirectResponse:
    ...
    session_id = _get_or_create_session(data_dir)
    _edit_session(session_id, enable_shuffling=False)
    return RedirectResponse(f"{SIDECAR_BASE_URL}/{session_id}/{target_url}", status_code=302)
...
def session_status(request: Request) -> dict[str, str | bool | None]:
    ...
    return {
        "active": bool(session_id and _sidecar_session_exists(session_id)),
...
def _get_or_create_session(data_dir: Path) -> str:
    session_id = _read_session_id(data_dir)
    if session_id and _sidecar_session_exists(session_id):
        return session_id

    session_id = _create_sidecar_session()
    _write_session_id(data_dir, session_id)
    return session_id


def _create_sidecar_session() -> str:
    _ensure_sidecar()
...
def _sidecar_session_exists(session_id: str) -> bool:
    _ensure_sidecar()
...
def _ensure_sidecar() -> None:
    ...
    with _sidecar_lock:
        if _sidecar_process is not None and _sidecar_process.poll() is None and _sidecar_ready():
            return
